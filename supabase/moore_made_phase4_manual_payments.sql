...
-- 5) Cash App / manual payment support.
-- Existing Stripe rows remain payment_method='stripe'. Manual payments are
-- recorded only by MFA-protected admin server routes.
...
alter table public.payments
  ...
  check (payment_method in ('stripe','cashapp','cash','check','other'));
