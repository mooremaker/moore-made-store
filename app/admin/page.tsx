...
type RequestRow = { id:string; customer_user_id:string|null; request_number:number; customer_name:string; email:string; phone:string|null; sms_consent:boolean; product:string; quantity:number; item_type:string|null; colors:string|null; sizes:string|null; logo_size:string|null; print_sides:string|null; placements:string[]|null; artwork_instructions:string|null; deadline:string|null; delivery:string|null; notes:string|null; requested_discount_code:string|null; order_items:StructuredOrderItem[]; shipping_address:ShippingAddress|null; reorder_source_request_id:string|null; reorder_price_lock:Record<string,unknown>|null; status:RequestStatus; payment_status:"unpaid"|"deposit_paid"|"paid"; amount_paid_cents:number; artwork_paths:string[]|null; artwork_rights_accepted:boolean; artwork_rights_accepted_at:string|null; artwork_rights_policy_version:string|null; artwork_rights_review_status:string; artwork_rights_review_note:string|null; artwork_rights_reviewed_at:string|null; tracking_number:string|null; tracking_url:string|null; fulfillment_note:string|null; fulfillment_notified_at:string|null; estimated_fulfillment_date:string|null; estimated_fulfillment_note:string|null; estimated_fulfillment_notified_at:string|null; estimated_fulfillment_notified_for_date:string|null; review_request_sent_at:string|null; cash_payment_request_status:"none"|"pending"|"contacted"|"completed"|"cancelled"; cash_payment_requested_at:string|null; cash_payment_requested_amount_cents:number|null; cash_payment_contacted_at:string|null; created_at:string; };
type ShowcaseRow = { id:string; customer_name:string; business_name:string|null; email:string; product:string; rating:number; review:string; caption:string|null; social_handle:string|null; status:ShowcaseStatus; homepage_featured:boolean; customer_primary:boolean; photo_paths:string[]|null; photo_preview_settings:unknown; created_at:string; };
...
export default async function AdminPage() {
  ...
  const { data, error } = await supabase
    ...
    .select("id,customer_user_id,request_number,customer_name,email,phone,sms_consent,product,quantity,item_type,colors,sizes,logo_size,print_sides,placements,artwork_instructions,deadline,delivery,notes,requested_discount_code,order_items,shipping_address,reorder_source_request_id,reorder_price_lock,status,payment_status,amount_paid_cents,artwork_paths,artwork_rights_accepted,artwork_rights_accepted_at,artwork_rights_policy_version,artwork_rights_review_status,artwork_rights_review_note,artwork_rights_reviewed_at,tracking_number,tracking_url,fulfillment_note,fulfillment_notified_at,estimated_fulfillment_date,estimated_fulfillment_note,estimated_fulfillment_notified_at,estimated_fulfillment_notified_for_date,review_request_sent_at,cash_payment_request_status,cash_payment_requested_at,cash_payment_requested_amount_cents,cash_payment_contacted_at,created_at")
    .order("created_at", { ascending: false });
  const requests = (data ?? []) as RequestRow[];
  const rowsWithFiles: AdminRequestRow[] = await Promise.all(requests.map(async (row) => ({
    ...row,
    is_admin_test_order: row.customer_user_id === auth.user.id || row.email.trim().toLowerCase() === (auth.user.email || "").trim().toLowerCase(),
  ...
  const { data: quoteRevisionData, error: quoteRevisionError } = await supabase
    ...
    .select("id,quote_id,revision_number,status,revision_reason,total_cents,estimated_profit_cents,estimated_margin_basis_points,proof_version,artwork_rights_confirmed_at,artwork_rights_policy_version,artwork_rights_snapshot,sent_at,responded_at,created_at")
    .order("revision_number", { ascending: false });
  type QuoteRevisionRow = { id:string; quote_id:string; revision_number:number; status:QuoteRecord["status"]; revision_reason:string|null; total_cents:number; estimated_profit_cents:number; estimated_margin_basis_points:number; proof_version:number; artwork_rights_confirmed_at:string|null; artwork_rights_policy_version:string|null; artwork_rights_snapshot:unknown|null; sent_at:string|null; responded_at:string|null; created_at:string; };
  ...
  const { data: showcaseData, error: showcaseError } = await supabase
    .from("showcase_posts")
    .select("id,customer_name,business_name,email,product,rating,review,caption,social_handle,status,homepage_featured,customer_primary,photo_paths,photo_preview_settings,created_at")
    .neq("status", "draft")
    .order("created_at", { ascending: false });
  ...
  const { data: paymentData, error: paymentError } = await supabase
    .from("payments")
    .select("id,request_id,quote_id,payment_kind,amount_cents,currency,status,payment_method,manual_reference,payer_name,payer_email,voided_at,void_reason,paid_at,created_at,receipt_number,receipt_token,receipt_order_number,receipt_payment_sequence,order_tax_cents,order_total_cents,stripe_fee_cents,stripe_net_cents,stripe_balance_transaction_id")
    .order("created_at", { ascending: false });
  ...
  const { data: productPricingData, error: productPricingError } = await supabase
    ...
    .select("product_slug,product_name,active,blank_cost_cents,size_blank_costs,size_customer_surcharges,print_cost_cents,additional_location_cost_cents,packaging_cost_cents,minimum_profit_per_item_cents,default_labor_hours,labor_rate_cents,target_margin_basis_points,tax_code,notes,created_at,updated_at")
  ...
  const { data: businessSettingsData, error: businessSettingsError } = await supabase
    ...
    .select("id,default_labor_rate_cents,minimum_labor_hours,pickup_address,default_tax_code,shipping_tax_code,margin_1_9_basis_points,margin_10_24_basis_points,margin_25_49_basis_points,margin_50_plus_basis_points,minimum_margin_floor_basis_points,standard_shirt_min_profit_cents,outsourced_min_margin_basis_points,overhead_basis_points,payment_fee_basis_points,payment_fee_fixed_cents,default_shipping_charge_cents,income_tax_reserve_basis_points,labor_warning_minutes_per_piece,weekly_sales_goal_cents,weekly_profit_goal_cents,weekly_owner_goal_cents,weekly_reserve_goal_cents,updated_at")
    .eq("id", "default")
    .maybeSingle();
