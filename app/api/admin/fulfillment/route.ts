...
export async function POST(request: Request) {
  ...
  try {
    ...
    const mode = body.mode === "shipped" ? "shipped" : body.mode === "pickup" ? "pickup" : "";
    if (!id || !mode) return NextResponse.json({ error: "Choose pickup or shipped." }, { status: 400 });
    ...
    const status = mode === "shipped" ? "shipped" : "ready";
    ...
    if (row.payment_status !== "paid") {
      return NextResponse.json({ error: "This order still has a balance due. Collect the remaining payment before marking it ready or shipped." }, { status: 409 });
    }

    const { error: updateError } = await supabase.from("custom_requests").update({
      status,
      tracking_number: mode === "shipped" ? trackingNumber || null : null,
      tracking_url: mode === "shipped" ? trackingUrl || null : null,
    ...
    const trackingHtml = mode === "shipped" && (trackingNumber || trackingUrl)
    ...
    const emailResult = await sendMooreMadeEmail({
      to: row.email,
      subject: mode === "shipped" ? `Your Moore Made order has shipped — ${reference}` : `Your Moore Made order is ready for pickup — ${reference}`,
      replyTo: process.env.MOORE_MADE_ADMIN_EMAIL,
      html: emailShell(
        mode === "shipped" ? "Your order is on the way." : "Your order is ready for pickup.",
        `<p style="line-height:1.65;margin:0 0 16px;">Hi ${escapeHtml(row.customer_name)}, your <strong>${escapeHtml(row.product)}</strong> order <strong>${escapeHtml(reference)}</strong> ${mode === "shipped" ? "has shipped." : "is ready for pickup."}</p>
