...
function CompactShowcaseCard({
  ...
  const [previewIndex] = useRefreshStablePhotoIndex(
    post.id,
    post.photoUrls
  );
  ...
  const previewPhoto =
    previewIndex == null
      ? null
      : post.photoUrls[previewIndex] ?? post.photoUrls[0] ?? null;

  const preview =
    previewIndex == null
      ? null
      : post.photoPreviews[previewIndex] ?? {
          x: 50,
          y: 50,
          zoom: 1,
        };
  ...
  return (
    <article className="homeCompactReview card">
      <button
        type="button"
        className="homeCompactReviewMedia"
        onClick={onOpen}
        aria-label={`Open review from ${displayName}`}
      >
        {previewPhoto ? (
          <img
            src={previewPhoto}
          ...
          <span
            className="homeCompactReviewLoading"
            aria-hidden="true"
          />
        ) : (
          <span className="homeCompactReviewFallback">
            Made by You
          </span>
        )}

        {post.photoUrls.length > 1 ? (
          <span className="homeCompactPhotoCount">
      ...
      <div className="homeCompactReviewBody">
        ...
        <div className="homeCompactReviewActions">
          <button
            type="button"
            className="textLink homeCompactOpen"
            onClick={onOpen}
          >
            View review →
          </button>

          <Link className="textLink" href="/custom-orders">
            Make something →
          </Link>
        </div>
      </div>
...
export function HomeShowcaseCarousel({
  ...
  const [selectedPost, setSelectedPost] =
    useState<PublicShowcasePost | null>(null);
    ...
    return (
      <div className="homeShowcaseCarousel homeShowcaseEmpty">
        ...
        <div className="homeShowcaseFooter">
          <span className="homeShowcaseCounter">
            No reviews yet
          </span>

          <Link
            className="textLink"
            href="/made-by-you/submit"
          >
            Share your order →
          </Link>
        </div>
  ...
  function move(direction: -1 | 1) {
    setStartIndex(
      (current) =>
        (current + direction + posts.length) %
        posts.length
    );
  }
  ...
  return (
    <div className="homeShowcaseCarousel homeShowcaseCompact">
      <div
        className="homeCompactReviewGrid"
      ...
      {posts.length > 1 ? (
        <div
          className="homeCompactMobileNav"
      ...
      {posts.length > DESKTOP_CARD_COUNT ? (
        <div
          className="homeCompactReviewNav"
          aria-label="Featured review controls"
        >
          <button
            type="button"
            className="showcaseArrow"
